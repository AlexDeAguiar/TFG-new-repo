...
public class PlayerMovementController : IController {
    ...
    public void update() {
		...
		if (characterController.isGrounded) {
			float mouseEvent = 0;
			if(Input.mouseScrollDelta.y > 0)	 { mouseEvent =  MOUSE_MOVEMENT; }
			else if(Input.mouseScrollDelta.y < 0){ mouseEvent = -MOUSE_MOVEMENT; }
			...
			float curSpeedX = mouseEvent != 0 ? mouseEvent : (canMove() ? speed * Input.GetAxis("Vertical") : 0);
		...
		if (canMove()) {
			...
			if (currRotationX <= 180 ){ //Looking down
	...
	private bool canMove() {
		return playerControllers.KeysEnabled && MoveKeysEnabled && !Input.GetKey(KeyCode.LeftAlt);
	}
}